...
const PacientDetail = props => {
  ...
  useEffect(() => {
    if (!data) {
      ...
      const getData = async () => {
        try {
          const userData = await Axios.get(`http://localhost:3000/pacientes/${cedula}`, {
            headers: { token: user.token },
          });
          const consultData = await Axios.get(
            `http://localhost:3000/consultas?pacient=${cedula}&full_data=true`,
            {
              headers: { token: user.token },
            }
          );
        ...
        <Col className="wrap">
          <Avatar className="wrap">
            <div className="image wrap">
              <img src="http://localhost:3000/img/userAvatar.png" alt="user avatar" />
            </div>
          </Avatar>
          <Info className="wrap">
