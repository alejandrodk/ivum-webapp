...
const PacientDetail = (props) => {
    ...
    useEffect(() => {
        ...
        const getData = async () => {
            ...
            let consult = Axios.get(`http://localhost:3000/consultas?pacient=${props.cedula}&full_data=true`);
            try {
                                ...
                                <div className="data">
                                    {
                                        consults.map((item, index) => (
                                            <ul className="wrap" key={index}>
                                                ...
                                                <li>{item.hora}</li>
                                                <li>{item.estado}</li>
                                                <li><i className="fas fa-file-invoice-dollar"></i></li>
                                                <li><i className="fas fa-file-invoice"></i></li>
                                            </ul>
                                        ))
                                    }
                                </div>
